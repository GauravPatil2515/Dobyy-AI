...
The integration of computational mathematics and artificial intelligence into the textile industry represents a critical frontier in modern manufacturing research. While generalized software disciplines have rapidly adopted cloud-centric, AI-native infrastructure, specialized industrial design tools such as those used in fabric production remain fundamentally reliant on decades-old computing paradigms. This literature survey investigates the historical progression of Computer-Aided Design (CAD) in textiles, thoroughly evaluates the current capabilities of Natural Language Processing (NLP) and Large Language Models (LLMs) in domain-specific workflows, and explores the intersection of computer vision within structural reverse-engineering. By reviewing current methodologies, this chapter explicitly isolates the research gaps preventing smaller manufacturing clusters, specifically within the Indian demographic, from accessing scalable, modern textile digitization software, bridging the necessity for the proposed Dobby Studio platform.
...
\section{Image Processing and Computer Vision for Fabric Analysis}

\subsection{Survey of Existing System}
Computational fabric analysis traditionally relies on programmatic color quantization algorithms. Techniques utilizing K-means clustering effectively sort raster pixels into dominant hex groupings, which has long been the standard for color segmentation in automated textile inspection \cite{ref8}. Conversely, advanced multimodal LLMs (like GPT-4V, LLaVA, and Llama 4 Scout) bring contextual understanding directly to Computer Vision. These systems don't just calculate pixel densities; they computationally "look" at an image and infer relational concepts, widely used to identify fabric defects or suggest matching attire in e-commerce avenues \cite{ref7}.

\subsection{Research Gaps}
Despite these advancements, a specific divergence occurs regarding structural fabric pattern reverse-engineering (tartan and dobby stripes). General color extraction models entirely ignore the spatial relationship of the warp and weft; they extract a palette without extracting the \textit{sett} (the order and proportion of those threads). Furthermore, current high-level multimodal vision models are generally not trained on identifying specific tartan nomenclatures or measuring thread-count proportionality directly from unstructured, potentially low-resolution user photographs.

\subsection{Problem Definition}
Reverse-engineering a fabric sett from a bare 2D image is functionally complex because the algorithm must differentiate between the structural weave indentations, the shadows cast by the illumination, and the foundational thread colors comprising the band widths. An accurate system must not only extract the dominant hues but also scan the geometry of the pattern sequentially, converting visual width proportionality into discrete numeric thread notations usable by a CAD matrix.

\subsection{Objectives}
To achieve visual fabric digitization, the targeted objectives are:
\begin{enumerate}
    \item To route base64 encoded user-uploaded assets directly into Groq's multimodal vision API for high-level semantic extraction.
    \item To prompt the vision model into determining accurate proportional thread arrays representing spatial color dimensions.
    \item To construct a local, fallback K-means clustering canvas parser when API limitations or network isolation occurs.
    \item To sanitize visual noise such that pixel variations are successfully grouped into consolidated, actionable textile stripes.
\end{enumerate}

\section{Weave Structure Mathematics and Tartan Sett Systems}

\subsection{Survey of Existing System}
The physical mechanics of weaving revolve around boolean algebra. Whether manipulating a manual dobby mechanism or a modernized electronic Jacquard loom, the intersection of warp (vertical) and weft (horizontal) threads requires binary mapping (shaft lifting plans and treadling drafts) \cite{ref9}. Furthermore, the historical notation of tartans—governed by institutions like the Scottish Tartan Registry—relies on specific mathematical repeat sequences dictating color ordering (e.g., forward and reflective pivots). The global engineering standard encapsulating these rules digitally is WIF 1.1, which rigidly structures arrays defining tie-ups, draft coordinates, and color indexing sequences \cite{ref10}. 
